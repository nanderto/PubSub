...
namespace Phantom.PubSub
{
    ...
    public class EsentStore<T> : IUnitOfWork, IDisposable, IEsentStore<T>
    {
        ...
        /// <summary>
        /// Adds the message in transaction.
        /// </summary>
        ...
        public int? AddMessageInTransaction(string body, string metadata)
        {
            ...
            using (var table = new Table(this.currentSession, this.dbid, this.messageTableName, OpenTableGrbit.None))
            {
                ...
                using (var internalTransaction = new Transaction(this.currentSession))
                {
                    ...
                    {
                        Api.SetColumn(this.currentSession, table, columnidMessage, body, Encoding.Unicode);
                        Api.SetColumn(this.currentSession, table, columnidMetaData, metadata, Encoding.Unicode);
